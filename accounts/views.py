...
def signup(request):
    ...
    if form.is_valid():
        ...
        try:
            ACTIVATION_KEY = activation_key_generator(20, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdegfhijklmnopqrstuvwxyz1234567890")
        ...
        messages.info(request, 'You have successfully registered, please activate your account from your email before signing in.')
                ...
                if user:
                    if user.is_active:
                        ...
                        if next:
                            ...
                            return HttpResponseRedirect(reverse('users.views.profile', args=(str(user_obj.username),)))
...
def reset_password_ask_email_page(request):
    ...
    if form.is_valid():
        ...
        if user_obj:
            ...
            messages.info(request, 'An email with the token has been send to the given email with a token. Enter that token here to proceed further')
